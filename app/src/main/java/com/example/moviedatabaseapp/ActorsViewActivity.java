...
public class ActorsViewActivity extends AppCompatActivity {
   ...
   /* protected void scrapeWebPage2()
    {
        try
        {
            URL url = new URL("https://www.imdb.com/name/"+selectedActor.getIMDB()+"/");
            Log.w("HTTP-URL","URL");
            HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
            Log.w("HTTP-Connect","HTTP URL");
            InputStream in = urlConnection.getInputStream();
            Log.w("HTTP","InputStream");
            InputStreamReader isr = new InputStreamReader(in);
            Log.w("HTTP","InputStreamReader");
            thePage = "";
            String str = "";
            int data = isr.read();
            while (data != -1)
            ...
            Log.w("HTTP","URL Connection - Disconnected");

            resultTextView.post(new Runnable() {
                public void run() {
                    resultTextView.setText(thePage);
                }
            });
        }
        catch (MalformedURLException ex)
        {
            Log.e("Malformed",Log.getStackTraceString(ex));
        }
        catch (IOException ex)
        {
            Log.e("IOException",Log.getStackTraceString(ex));
        }
        catch(Exception ex)
        {
            Log.e("Exception",Log.getStackTraceString(ex));
        }
    } // protected void scrapeWebPage()*/
